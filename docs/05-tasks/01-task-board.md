...
| --- | --- | --- | --- | --- | --- | --- |
| `SETUP-001` | Tạo cấu trúc solution backend ASP.NET Core Web API .NET 8 | N/A | `[ ]` | P0 | N/A | Có solution backend, project API, Application/Domain/Infrastructure nếu dùng clean architecture |
| `SETUP-002` | Tạo project test backend | N/A | `[ ]` | P0 | `SETUP-001` | Có project unit/integration test và chạy được lệnh test |
| `SETUP-003` | Cấu hình SQL Server connection string theo môi trường dev | N/A | `[ ]` | P0 | `SETUP-001` | App đọc được config, không hard-code secret production |
| `SETUP-004` | Cấu hình Swagger/OpenAPI cho `/api/v1` | N/A | `[ ]` | P0 | `SETUP-001` | Swagger hiển thị các nhóm API chính |
| `SETUP-005` | Cấu hình response lỗi ProblemDetails và validation pipeline | N/A | `[ ]` | P0 | `SETUP-001` | API lỗi trả format thống nhất như API spec |
| `SETUP-006` | Cấu hình JWT authentication và authorization policy cơ bản | `FR-001`, `FR-002` | `[ ]` | P0 | `SETUP-001` | Endpoint protected yêu cầu token hợp lệ |
| `SETUP-007` | Cấu hình CORS cho Flutter app/dev environment | N/A | `[ ]` | P1 | `SETUP-001` | Mobile/debug client gọi được API |
| `SETUP-008` | Tạo Flutter project mobile UniShare | N/A | `[ ]` | P0 | N/A | App Flutter chạy được trên Android emulator/device |
| `SETUP-009` | Cấu hình flavor/env cho Flutter gọi backend dev/staging | N/A | `[ ]` | P1 | `SETUP-008` | App đổi được base API URL theo môi trường |
